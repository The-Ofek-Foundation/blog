...
There is no trickery here; we literally have the limit of the exact sequence in question, with
...
[^1]: For those interested, one of the ways to define the number $$e$$ is as the limit of the sequence $$\left ( 1 + \frac{1}{n} \right )^n$$ as $$n$$ approaches $$\infty$$. Here is some very hand-wavy intuition: $$(1 - \frac{1}{x})^x = \left [ (1 + \frac{1}{-x})^{-x} \right ]^{-1} = \left [ (1 + \frac{1}{n})^{n} \right ]^{-1} \to [ e ]^{-1}$$, where we substituted $$n$$ for $$-x$$.

[^2]: Wikipedia didn't enumerate these points, I did to make it easier to reference them.
